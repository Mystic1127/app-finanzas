...
object SuggestionService {
    ...
    suspend fun suggest(ctx: Context, nota: String?, esIngreso: Boolean, monto: Double): CategorySuggestion =
        withContext(Dispatchers.IO) {
            ...
            trans.forEach { t ->
                ...
                if (!nota.isNullOrEmpty() && !t.nota.isNullOrEmpty() && t.nota.lowercase().contains(nota.lowercase())) {
    ...
    fun suggest(ctx: Context, nota: String?, esIngreso: Boolean, monto: Double, cb: Callback) {
        scope.launch {
            runCatching { suggest(ctx, nota, esIngreso, monto) }
